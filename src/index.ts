...
function createHead(x: number, y: number) {
  return Matter.Bodies.circle(x, y, headRadius, {
    ...createBodyOptions(),
    inertia: Infinity,
  });
 }
...
class Individual {
  ...
  constructor(id: number) {
    ...
    //create head
    //this.nodes.push(
   //    createHead((sideLength / 2) *  (segments+2)+ startX, sideLength * 0.866 /2 + startY )
  //   );
...
const sketch = (p: p5) => {
  ...
  p.preload = function () {
    img = p.loadImage("tile3.png");
    img2 =p.loadImage("forest.jpeg");
...

    {
      ...
      for (const c of GA.population) {
        if (params.showIndividual < 0 || c.id == params.showIndividual) {
          ...
          //p.stroke("white");
          //p.strokeWeight(4);
          p.stroke(colorAlpha(palletes[c.id % palletes.length], 0.8));
          p.strokeWeight(4);
          ...
          // Draw nodes.
          p.noStroke();
          p.fill(colorAlpha("white", 0.8));
          for (const n of c.nodes) {
            p.circle(n.position.x, n.position.y, n.circleRadius * 2);
          }
//draw head
          //p.noStroke();
          //p.fill(colorAlpha(palletes[c.id % palletes.length], 1));
          // hole
          p.stroke(colorAlpha(palletes[c.id % palletes.length], 1));
          p.strokeWeight(6);
          p.circle((c.nodes[3].position.x+c.nodes[4].position.x)/2+15, (c.nodes[3].position.y+c.nodes[4].position.y)/2-10, headRadius * 2);
